(() => {
  ...
  const state = { data: null, runName: '', frame: 0, batch: 0, coordinate: 0, selected: null, playing: false, lastTick: 0, layout: [], job: null, jobTimer: null, live: null, liveModels: [], demo: null, demoLastTick: 0, embodiedCheckpointVersion: '', embodiedJobId: '' };
  ...
  async function refreshEmbodiedModels() {
    ...
    try {
      ...
      runs.runs.forEach((run) => { const unit = run.training_mode === 'batch' ? 'gen' : 'tick', status = run.complete ? 'complete' : `checkpoint ${run.checkpoint_tick}/${run.ticks} ${unit}`, algorithm = run.algorithm === 'genetic' ? 'GA' : 'CMA-ES', lifetime = String(run.objective || '').includes('lifetime'), score = lifetime ? `prey ${number(run.prey_best_lifetime)} ticks · predator ${number(run.predator_best_lifetime)} ticks` : `legacy shaped score ${number(run.prey_best_lifetime)}`; ui.embodiedContinueRun.append(new Option(`${run.id.slice(0, 8)} · ${run.training_mode || 'continuous'} · ${algorithm} · ${status} · ${score}`, run.id)); });

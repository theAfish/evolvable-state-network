...
class EmbodiedFoodWebTrainingPayload(StrictModel):
    ...
    # 0 means automatic selection; explicit worker counts are bounded only by
    # the operating system/runtime rather than an arbitrary API ceiling.
    workers: int = Field(0, ge=0)
    body_inputs: tuple[Literal["hunger", "energy_change", "ate", "time_since_meal"], ...] = Field(("hunger",), min_length=1)
    population_size: int = Field(24, ge=2)
    prey_count: int = Field(5, ge=1)
    predator_count: int = Field(2, ge=0)
    hidden_nodes: int = Field(31, ge=1)
    state_width: int = Field(2, ge=2)
    node_hidden_layers: tuple[int, ...] = Field((8,), min_length=1)
    node_activation: Literal["tanh", "relu", "gelu", "silu"] = "tanh"
    edge_hidden_layers: tuple[int, ...] = Field((12,), min_length=1)
    edge_activation: Literal["tanh", "relu", "gelu", "silu"] = "tanh"
    edge_latent_width: int = Field(3, ge=1)
    mean_degree: float = Field(6.0, ge=0)
    allow_input_output_connections: bool = False
    initial_state_scale: float = Field(.12, ge=0)
    network_dt: float = Field(.05, gt=0)
    max_delta: float = Field(.12, gt=0)
    edge_step_scale: float = Field(.06, gt=0)
    rule_output_scale: float = Field(1.0, gt=0)
    mutation_sigma: float | None = Field(None, gt=0)
    elite_fraction: float = Field(.25, gt=0, lt=1)
    immigrant_fraction: float = Field(.25, ge=0, lt=1)
    immigrant_sigma: float | None = Field(None, gt=0)
    immigrant_mode: Literal["zero", "population"] = "zero"
    local_mutation_sigma: float | None = Field(None, gt=0)
    local_offspring_fraction: float | None = Field(None, ge=0, lt=1)
    regional_fraction: float = Field(0.0, ge=0, lt=1)
    regional_scale: float = Field(1.0, gt=0)
    regional_min_std: float = Field(.02, gt=0)
    global_fraction: float = Field(0.0, ge=0, lt=1)
    global_parameter_range: float = Field(1.0, gt=0)
    global_viability_filter: bool = False
    global_max_sampling_attempts: int = Field(20, ge=1)
    ...
    batch_episode_steps: int = Field(256, ge=8)
    batch_trials: int = Field(4, ge=1)
    batch_validation_trials: int = Field(8, ge=1)
    batch_test_trials: int = Field(16, ge=1)
    batch_opponents: int = Field(2, ge=1)
    enforce_survival_pressure: bool = True

    @model_validator(mode="after")
    def validate_degree(self) -> "EmbodiedFoodWebTrainingPayload":
        if any(width < 1 for width in self.node_hidden_layers + self.edge_hidden_layers):
...
class EmbodiedDemoPayload(StrictModel):
    ...
    prey_count: int | None = Field(None, ge=1)
    predator_count: int | None = Field(None, ge=0)
    initial_food: int | None = Field(None, ge=0)
    max_food: int | None = Field(None, ge=0)
    food_growth_rate: float | None = Field(None, ge=0)
